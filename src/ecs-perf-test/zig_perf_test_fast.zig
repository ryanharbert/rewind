...
// Fast system that bypasses ECS API like C# version
fn fastUpdateTransformSystem(frame: *PerfECS.Frame) !void {
    // Get the query bitset (like C#)
    var query = try frame.query(&.{ Transform, Velocity });
    const delta_time = frame.deltaTime;
    
    // Reset the query to get a fresh iterator
    query.reset();
    
    // Iterate through the bitset directly and access components by entity ID
    while (query.next()) |result| {
        const entity = result.entity;
        
        // Direct component access (similar to C# direct array access)
        if (frame.getComponent(entity, Transform)) |transform| {
            if (frame.getComponent(entity, Velocity)) |velocity| {
                // Update position (in-place modification)
...
// Fast damage system
fn fastDamageSystem(frame: *PerfECS.Frame) !void {
    ...
    while (query.next()) |result| {
        ...
        if (frame.getComponent(entity, Health)) |health| {
            health.current -= 1;
...
pub fn main() !void {
    ...
    std.debug.print("=== Zig Bitset ECS Performance Test (Fast Version - C# Style) ===\n", .{});
    ...
    for (entity_counts) |entity_count| {
        ...
        // Create entities and track first entity for verification
        ...
        for (0..100) |_| {
            ...
            try fastUpdateTransformSystem(&perf_ecs.current_frame);
            try fastDamageSystem(&perf_ecs.current_frame);
        }
