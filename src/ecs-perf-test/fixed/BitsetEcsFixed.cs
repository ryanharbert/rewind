...
namespace BitsetEcsFixed
{
    ...
    public class ComponentStorage<T> where T : struct
    {
        ...
        public T[] DenseArray => _dense; // Expose for direct access
        public uint[] EntityToIndexArray => _entityToIndex; // Expose for direct access
    ...
    {
        public ComponentStorage<Transform> Transforms { get; }
        public ComponentStorage<Velocity> Velocities { get; }
        public ComponentStorage<Health> Healths { get; }
        ...
        {
            _maxEntities = maxEntities;
            Transforms = new ComponentStorage<Transform>(maxEntities);
            Velocities = new ComponentStorage<Velocity>(maxEntities);
            Healths = new ComponentStorage<Health>(maxEntities);
        ...
        {
            Healths.Add(entity, component);
        }

        // Query for Transform + Velocity - returns the bitset for manual iteration
        public ref BitSet QueryTransformVelocity()
        {
            // Copy active entities to result
            _activeEntities.IntersectWith(_activeEntities, ref _queryResult);
            
            // Intersect with transforms
            Transforms.EntityBitset.IntersectWith(_queryResult, ref _queryTemp);
            (_queryResult, _queryTemp) = (_queryTemp, _queryResult);
            
            // Intersect with velocities
            Velocities.EntityBitset.IntersectWith(_queryResult, ref _queryTemp);
            (_queryResult, _queryTemp) = (_queryTemp, _queryResult);

            return ref _queryResult;
        }

        // Query for Health only
        public ref BitSet QueryHealth()
        {
            _activeEntities.IntersectWith(Healths.EntityBitset, ref _queryResult);
            return ref _queryResult;
        }
    ...
    // Systems - now directly modify components
    public static class Systems
    {
        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        public static void UpdateTransformSystem(ECS ecs)
        {
            var queryBitset = ecs.QueryTransformVelocity();
            float deltaTime = ecs.DeltaTime;
            
            // Direct array access for maximum performance
            var transforms = ecs.Transforms.DenseArray;
            var velocities = ecs.Velocities.DenseArray;
            var transformEntityToIndex = ecs.Transforms.EntityToIndexArray;
            var velocityEntityToIndex = ecs.Velocities.EntityToIndexArray;
            ...
            foreach (int entity in queryBitset)
            {
                ...
                ref Transform transform = ref transforms[transformIndex];
                ref readonly Velocity velocity = ref velocities[velocityIndex];
        ...
        {
            var queryBitset = ecs.QueryHealth();
            
            // Direct array access
            var healths = ecs.Healths.DenseArray;
            var healthEntityToIndex = ecs.Healths.EntityToIndexArray;

            foreach (int entity in queryBitset)
            {
                uint healthIndex = healthEntityToIndex[entity];
                ref Health health = ref healths[healthIndex];
    ...
    class Program
    {
        static void Main()
        {
            Console.WriteLine("=== C# Bitset ECS Performance Test (Fixed Version) ===");
            ...
            foreach (int entityCount in entityCounts)
            {
                ...
                // Capture initial position to verify work is done
                float initialX = ecs.Transforms.Get(firstEntity).X;
                int initialHealth = ecs.Healths.Has(firstEntity) ? ecs.Healths.Get(firstEntity).Current : -1;
                ...
                float finalX = ecs.Transforms.Get(firstEntity).X;
                int finalHealth = ecs.Healths.Has(firstEntity) ? ecs.Healths.Get(firstEntity).Current : -1;
                ...
                // Verification
                Console.WriteLine($"Transform verification - Initial X: {initialX:F2}, Final X: {finalX:F2}, Delta: {finalX - initialX:F2}");
                if (initialHealth >= 0)
                {
                    Console.WriteLine($"Health verification - Initial: {initialHealth}, Final: {finalHealth}");
                }
            }
